    ...
    public function drawMainMenu()
    {
        ...
        foreach ($catListTop as $catTop) :
            ...
            if($isDropdown){ // Draw Dropdown Menu
				...
				$desktopBgImg = $this->getBgImage($catTop);
				$childHtml = $this->getTreeCategoriesExt($idTop,$desktopBgImg); // include magic_label
...
//								$url =  '<a href="'. $child->getUrl().'"><span>'.$this->__($child->getName()) . '</span></a>';
...
//					$desktopBgImg = $this->getBgImage($catTop);
//					if($desktopBgImg) :
//						$desktopTmp .= '<div style="width:400px;height:200px;background:url(\''.$desktopBgImg.'\') no-repeat">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>';  /* Add Cate image */
//					endif;
...
//			endif;
			if($hasChild || $blocks['top'] || $blocks['left'] || $blocks['right'] || $blocks['bottom']) :
				$desktopTmp .= '<div class="level-top-mega" style="display:none">';  /* Wrap Mega */
					$desktopTmp .='<div class="content-mega">';  /*  Content Mega */
						$desktopTmp .= $blocks['top'];
						$desktopTmp .= '<div class="content-mega-horizontal">';
							$desktopTmp .= $blocks['left'];
    ...
    public function getCatTop()
    {
        $collection = Mage::getModel('magicmenu/category')->getCollection()
                        ->addAttributeToSelect(array('entity_id','name','magic_label','short_desc','url_path','magic_thumbnail','magic_image'))
    ...
    public function getBgImage($object)
    {
        if($file = $object->getMagicImage()){
            $image  = Mage::helper('magicmenu/image');
            $width  = 400; //$this->config['widthThumb'];
            $height = 200; //$this->config['heightThumb'];
            $img    = $image->resizeImg('/'.$file, $width, $height);
            if($img) return $img;
        }
    }
